...
function App() {
  ...
  const getForexData = async () => {
    const response = await axios.get(
      `https://v6.exchangerate-api.com/v6/ff5446d657010e0e02233999/pair/${currOne}/${currTwo}`
    );
    console.log(response.data.conversion_rate);
    setForexRate(response.data.conversion_rate);
  };

  const currOneSelect = (e) => {
    setCurrOne(e.target.value);
  };

  const currTwoSelect = (e) => {
  ...
  const resultHandler = () => {
    let result =
      // currOne === currTwo ? inputOne / forexRate : forexRate * inputOne;
  ...
  return (
    <div className='App'>
      ...
      <div className='currency'>
        <select id='currency-one' onChange={currOneSelect} value={currOne}>
      ...
      <div className='currency'>
        <select id='currency-two' onChange={currTwoSelect} value={currTwo}>
          <option value='AFN'>Afghanistan</option>
          <option value='ALL'>Albania</option>
          <option value='DZD'>Algeria</option>
          <option value='AOA'>Angola</option>
      ...
      <div>
        <button onClick={resultHandler}>Convert</button>
      </div>
    </div>
  );
}
